...
class GreedyBestFirstSearch:
    ...
    def search(grid: Grid) -> Solution:
        ...
        h = GreedyBestFirstSearch.h(root.state, grid.end)
        frontera = PriorityQueueFrontier()
        
        # Añadir la heurística como prioridad
        frontera.add(root, h)
        ...
        while True:
            ...
            for accion in grid.actions(nodo.state):
                ...
                if sucesor not in reached or nuevo_costo < reached[sucesor]:
                    ...
                    h_act = GreedyBestFirstSearch.h(sucesor, grid.end)

                    # GBFS: añadir a la frontera SOLO con la heurística (no sumar costo)
                    frontera.add(Nhijo, h_act)
